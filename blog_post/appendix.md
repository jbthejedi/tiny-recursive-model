...
The "heirarchy" means high and low frequencies
...
#### Details
By the Implicit Function Theorem [Krantz & Parks, 2002] with the 1-step gradient approximation, backpropogation uses only the gradients from the last two steps, $f_L$ and $f_H$, in `hrm` of *Figure 1*.
...
#### Concern
The author of TRM raises concerns that it is unlikely a fixed point will be reached in the HRM training because 1) there are no checks an equilibrium is reached (no checks on the residual), and 2) there are only 4 forward passes of $f_L$ and $f_H$, not (need more here).

TRM bypasses the need for IFT/1-step gradient approximation entirely.
...
#### Concern
The author mentions that ACT makes deep supervision more efficient by decreasing the number of steps per batch, but it requires two calls to `hrm` in a single forward pass. Not only that, imo, ACT is just complicated and weird.
